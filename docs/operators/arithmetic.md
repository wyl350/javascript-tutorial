...
false + 'a' // "falsea"
```
**字符串的权重大写。以第一个加数是字符串或者数字为准**

加法运算符是在运行时决定，到底是执行相加，还是执行连接。也就是说，运算子的不同，导致了不同的语法行为，这种现象称为“重载”（overload）。由于加法运算符存在**重载**，可能执行两种运算，使用的时候必须很小心。
...
自增和自减运算符有一个需要注意的地方，就是放在变量之后，会先返回变量操作前的值，再进行自增/自减操作；放在变量之前，会先进行自增/自减操作，再返回变量操作后的值。**(副作用在前还是副作用在后的问题。)**
...
数值运算符号和负数值运算符，都会返回一个新的值，而不会改变原始变量的值。**（也就是没有副作用。）**
