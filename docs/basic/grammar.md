...
`switch`语句部分和`case`语句部分，都可以使用表达式。**注意：这里的说的这个表达式，必须是返回或者表示一个值。不能是语句。例如变量或者明确的值，都是表达式的具体体现。**
...
需要注意的是，`switch`语句后面的表达式，与`case`语句后面的表示式比较运行结果时，采用的是严格相等运算符（`===`），而不是相等运算符（`==`），这意味着**比较时不会发生类型转换**。
...
```javascript
(条件) ? 表达式1 : 表达式2  **注意这里的表达式1和2 就是返回值。**
```
...
`for`语句后面的括号里面，有三个表达式。**我这里理解表达式可以理解为两类，一类是条件表达式，就是指这个表达式的返回值为布尔值，另一类表达式的返回值为非布尔值，这里总结循环语句必须有三个表达式，这三个表达式都必须有。其中初始化表达式，仅仅是执行一次，而条件表达式和递增表达式是每次循环都会执行的。**
...
上面代码只会执行10次循环，一旦`i`等于10，就会跳出循环。**特点是，抠出循环前面的部分。**

`for`循环也可以使用`break`语句跳出循环。**直接跳出，不设定跳出条件。当然也是可以设定跳出条件的。但是设定跳出条件，就多此一举了。**

**break 的特点就是只能保留循环前面的部分，不能任意的保留循环的其他部分。因此，需要使用 continue 。**
...
**因此，在每次循环体重，continue 的位置后面不能没有代码语句。可以随意的抠出循环中的值。**
...
如果存在多重循环，不带参数 **（这里我看不懂，为什么是不带参数）** 的`break`语句和`continue`语句都只针对最内层循环。

### 标签（label）

JavaScript 语言允许，语句的前面有标签（label），相当于**定位符**，用于跳转到程序的任意位置，标签的格式如下。
...
foo: { // 代码块是可以命名的！真是霸气
  console.log(1);
  break foo; // 注意这里的这个break 是不能单独使用的，必须加上一个跳转出去的语句 foo，否者就会出错，因为这里的这个 break 这个语句不在循环或者条件语句中。
  console.log('本行不会输出');
}
console.log(2);
// 1
