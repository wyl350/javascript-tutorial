...
除了用`function`命令声明函数，还可以采用**变量赋值**的写法。
...
这种写法将一个匿名函数赋值给变量。这时，这个匿名函数又称函数表达式（Function Expression），因为赋值语句的等号右侧只能放表达式。（**既是函数又是表达式。因此叫做函数表达式。**）
...
**声明提前，调用放后。这叫做函数声明提升，和变量的声明提前是一样的。**
...
上面代码定义了空函数`f`，它的`length`属性就是定义时的参数个数。不管调用时输入了多少个参数，`length`属性始终等于2。**（函数的属性就是函数本身的属性，所以跟调用是没有关系的。）**
...
对于那些**原生的函数**，`toString()`方法返回`function (){[native code]}`。 **原生函数应该是做了一层处理。**
...
函数参数不是必需的，*JavaScript 允许省略参数。(也就是也可函数定义参数，但是函数本身又不使用参数。霸气。)*
...
f(1, 2, 3) // 1
f(1) // 1
f() // undefined   调用的时候，必须提供对应声明时候的参数。
...
总之，`eval`的本质是在**当前作用域之中**，注入代码。由于安全风险和不利于 JavaScript 引擎优化执行速度，所以一般不推荐使用。通常情况下，`eval`最常见的场合是解析 JSON 数据的字符串，不过正确的做法应该是使用原生的`JSON.parse`方法。
