...
如**果一个对象的所有键名都是正整数或零，并且有`length`属性，那么这个对象就很像数组，语法上称为“类似数组的对象”**（array-like object）。
...
上面代码为对象`obj`添加了一个数字键，但是`length`属性没变。这就说明了`obj`不是数组。而数组是可以变得。**难道这就是数组和对象的唯一区别吗？**
