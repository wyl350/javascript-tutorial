...
在`if`语句中，它们都会被自动转为`false`，相等运算符（`==`）甚至直接报告两者相等。**false null undefined 三个在if语句中都是转变为false，之间是‘==’的关系。**
...
**预期为布尔值，那么就应该转变为布尔值，但是这里的变量不会转变，只是他的返回最终值为布尔值。**

- `undefined`
- `null`
- `false`
- `0`
- `NaN`   **undefined 转变为数字为NAN**
- `""`或`''`（空字符串）
